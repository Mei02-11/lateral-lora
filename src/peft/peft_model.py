...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    ##### Print all adapters currently loaded in model
    ##### Stage 2 debugging helper
    def print_available_adapters(self):
        print("\n=== AVAILABLE ADAPTERS ===")
    ...
    ##### Store old adapters inside every LoRA layer
    ##### Used later for lateral transfer
    ##### Stage 2 preparation only
    def set_old_adapters(self,adapter_list):
        for module in self.modules():
            if hasattr(module,"selected_old_adapters"):
                module.selected_old_adapters=adapter_list

    ##### Stage 7.5 Dynamic
    ##### Store adapter embeddings
    def set_adapter_embedding(
        self,
        adapter_name,
        embedding
    ):
        ##### Create embedding dictionary
        if not hasattr(self,"adapter_embeddings"):
            self.adapter_embeddings={}

        ##### Store embedding
        self.adapter_embeddings[adapter_name]=embedding.detach()
        print(f"\n[EMBEDDING STORED] {adapter_name}")

    ##### Stage 7
    ##### Top-K adapter routing
    ##### Simple prototype version
    def select_topk_adapters(
        self,
        current_adapter,
        eval_dataset=None,
        k=3,
        ...
        if (
            not hasattr(self,"adapter_embeddings")
            or len(self.adapter_embeddings)==0
        ):
            return [],{}

        current_embedding=self.current_task_embedding
        similarities={}

        ##### Cosine similarity prefilter
        for adapter_name,embedding in self.adapter_embeddings.items():
            ...
            print(f"{adapter_name}: {similarity:.4f}")
        ...
        baseline_loss=baseline_total_loss/baseline_steps
        print(f"[BASELINE LOSS] {baseline_loss:.4f}")

        ##### Enable lateral transfer ONLY for routing
        for module in self.modules():
        ...
        for old_adapter in candidate_adapters:
            print(f"\n[TESTING TRANSFER] {old_adapter}")

            for module in self.modules():
                if hasattr(module,"selected_old_adapters"):
                    module.selected_old_adapters=[old_adapter]
            ...
            improvement=baseline_loss-avg_loss

            adapter_scores[old_adapter]=improvement

            print(
                f"[TRANSFER IMPROVEMENT] "
                f"{old_adapter}: "
                f"{improvement:.10f}"
            )

        ##### Threshold-based selection
        ranked_adapters=sorted(
        ...
        for adapter,score in ranked_adapters:
            if score>min_improvement:
                selected.append(adapter)
                print(
                    f"[SELECTED] "
                    f"{adapter} "
                    f"(improvement={score:.4f})"
                )
            else:
                print(
                    f"[REJECTED] "
        ...
        for module in self.modules():
            if hasattr(module,"selected_old_adapters"):
                module.selected_old_adapters=selected

        ##### Disable lateral transfer after routing
        for module in self.modules():
            if hasattr(module,"enable_lateral_transfer"):
                module.enable_lateral_transfer=False

        print(f"\n[FINAL SELECTED] {selected}")

        return selected,adapter_scores
